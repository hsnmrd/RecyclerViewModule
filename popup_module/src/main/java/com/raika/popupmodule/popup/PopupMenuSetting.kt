...
internal class PopupMenuSetting(var listener: ((view: View, popupWindow: PopupWindow?, dimPopupWindow: PopupWindow?) -> Int?)) {
    ...
    fun showPopup(context: Activity, targetView: View, layout: Int, isAutoClick: Boolean = true, xTouch: Float = -1f, yTouch: Float = -1f): PopupWindow? {
        ...
        if (isAutoClick) {
            ...
            viewParent?.let {
                ...
                hideKeyboard(context)
                if (xTouch == -1f && yTouch == -1f) {
                    if (recyclerHeight + targetView.y + targetView.height > screenHeight - 70) {
                        popupWindow?.showAsDropDown(targetView, (xTouch.toInt()), (yTouch - targetView.height - recyclerHeight).toInt())
                    } else {
                        popupWindow?.showAsDropDown(targetView, (xTouch.toInt()), (yTouch - targetView.height).toInt())
                    }
                } else {
                    if (recyclerHeight + targetView.y + targetView.height > screenHeight - 70) {
                        popupWindow?.showAsDropDown(targetView, (x), (y - targetView.height - recyclerHeight))
